...
import (
	...
	"github.com/zerok/webmentiond/frontend"
...
func newServeCmd() Command {
	...
	var serveCmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if uiPath == "" || uiPath == ":embedded" {
				uiFileSystem = frontend.FS
			} else {
				uiPath, err = filepath.Abs(uiPath)
				if err != nil {
					return err
				}
				uiFileSystem = os.DirFS(uiPath)
			}
	...
	serveCmd.Flags().String("ui-path", "", "Path which should be served as /ui/ (default: use embedded UI)")
